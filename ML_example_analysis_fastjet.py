...
model.fit(
    [jet_inputs, jet_eta], m_pred_baseline,
    batch_size=32, 
    epochs=20, 
...
plt.hist(m_pred_NN/m_pred_baseline, bins=100, range=[0, 2], histtype='step', label=f'NN: stdev={np.std(m_pred_NN/m_pred_baseline):.3f}')
plt.legend()
plt.xlabel('NN / fastjet mass')
plt.savefig('results_train_fastjet.png')
plt.clf()
...
plt.legend()
plt.xlabel('mass')
plt.savefig('distrib_mass_train_fastjet.png')
plt.clf()
